...
#define kTopConstraintWithOutGender 10
...
@implementation FilterSearchViewController
{
    ...
    TabSlideView *  styleAndFeaturesSlideView;
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    if (appDelegate.bLoadingFilterInfo)
    {
        ...
        //        // already loading information from server, show the activity
    }
    else
    {
        if (appDelegate.bLoadedFilterInfo)
        {
        ...
        //        else
        //        {
        //            [self startLoadingInfoFromServer];
        //        }
    }
...
-(void) createGenderSlideView
{
    if (genderSlideView == nil)
    {
        ...
        // maybeout of the if block
//        [arr addObject:NSLocalizedString(@"_MEN_",nil)];
//        [arr addObject:NSLocalizedString(@"_WOMEN_",nil)];
//        [arr addObject:NSLocalizedString(@"_BOYS_",nil)];
//        [arr addObject:NSLocalizedString(@"_GIRLS_",nil)];
//        [arr addObject:NSLocalizedString(@"_KIDS_",nil)];
        //        [arr addObject:@"Current Collection"];
        //        [arr addObject:@"Past Collection"];
        
        [genderSlideView initSlideButtonWithButtons:arr andDelegate:self];
    }
    
    fPosY += kHeightTopFilter + kMarginBetweenBars;
}
...
-(void) showViewStyleAndFeature:(BOOL) bAnimated
{
    ...
    styleAndFeaturesSlideView.hidden = NO;
}
...
-(void) showStylesAndFeatureGroup
{
    if (selectedProductCategory != nil)
    {
        ...
        TabSlideView *subproductCategoryTabSlideView = styleAndFeaturesSlideView;
...
-(TabSlideView *) createTabSlideViewGroup: (long) iLevel withLevelPosY:(long) iLevelPos
{
    CGRect sliderFrame = [self getRectForViewAtLevel:iLevelPos];
    
    TabSlideView * _slideTabView = [[TabSlideView alloc] initWithFrame:sliderFrame];
    ...
    _slideTabView.colorSelectedTextButtons = [UIColor darkGrayColor];
    _slideTabView.colorBackgroundButtons = [UIColor clearColor];
    _slideTabView.colorBackgroundSelectedButtons = [UIColor colorWithRed:(229/255.0) green:(229/255.0) blue:(229/255.0) alpha:1.0];
    _slideTabView.fShadowRadius = 0.0;
    _slideTabView.colorShadowButtons = [UIColor clearColor];
    _slideTabView.leftMarginControl = 3;
    _slideTabView.paddingButton = 6;
    _slideTabView.spaceBetweenButtons = 0;
    _slideTabView.alphaButtons = 1;
    _slideTabView.bMultiselect = NO;
    _slideTabView.bSeparator = YES;
    _slideTabView.typeSelection = TAB_TYPE;
    _slideTabView.bUnselectWithClick = NO;
...
- (void)slideButtonView:(SlideButtonView *)slideButtonView btnClick:(int)buttonEntry
{
    ...
    else if(slideButtonView == styleAndFeaturesSlideView)
    {
        // select the new product catgeory
        iTabSelected = buttonEntry;
        
        if (iTabSelected < iNumSubProductCatgeoriesShowing)
        {
            //            [self getDataSubProductCategoryAddingToSearchTerms:YES andAnimated:YES];
            [self showSubProductCategoryCollectionViewRecursive:0];
            [self.mainCollectionView setContentOffset:CGPointMake(0.0f, 0.0f) animated:NO];

        }
        else if (iTabSelected == iNumSubProductCatgeoriesShowing)
        {
            // brands
            //            [self getDataBrandAddingToSearchTerms:YES andAnimated:YES];
            [self showBrandsProductCategoryCollectionView:0];
            [self.mainCollectionView setContentOffset:CGPointMake(0.0f, 0.0f) animated:NO];
        }
        else if (iTabSelected > iNumSubProductCatgeoriesShowing)
        {
            // featureGroups
            [self getDataFeatureGroupAddingToSearchTerms:YES andAnimated:YES];
            [self showFeatures:0];
            [self.mainCollectionView setContentOffset:CGPointMake(0.0f, 0.0f) animated:NO];
        }
...
//        else
//        {
//            backLabelColor = [UIColor colorWithRed:(234/255.0) green:(234/255.0) blue:(234/255.0) alpha:1.0f];
//        }
    ...
    if ([cellType isEqualToString:@"FeatureCell"] || [cellType isEqualToString:@"ProductCategoryCell"])
    {
        ...
        else if (self.viewSubProductAndFeatures.hidden == NO)
        {
            ...
            else
            {
                ...
                {
                    [self addSearchTerm:sNameTrimmed];
                    if (_bAutoSwap)
                    {
                        if (iTabSelected < [featureGroupsSelectedProductCategory count] + iNumSubProductCatgeoriesShowing)
                        {
            ...
            /*
             ...
             {
             productCategory = [childrenSelectedProductCategory objectAtIndex:[indexPath indexAtPosition:1]];
             NSLog(@"Button clicked: %@.", productCategory.name);
             sName = productCategory.name;
             }
             else if (brandsSelectedProductCategory != nil)
             {
             ...
             {
             // a subproductcatgeory has been selected
             [self addSearchTermOfProductGroup:productCategory];
             [self.mainCollectionView reloadItemsAtIndexPaths:@[indexPath]];
             }
             else if (brand != nil)
             {
