...
- (void)viewDidLoad
{
    ...
    self.inspireViewHeightConstraint.constant = kInspireViewHeight;
    if (_bFromTagSearch) {
        self.inspireView.hidden = NO;
        self.inspireImageView.image = self.inspireImage;
    }
    else {
...
-(void)hideInspireView {
    ...
    [UIView animateWithDuration:0.8
                     ...
                     completion:^(BOOL finished){
                         ...
                         self.inspireImageView.hidden = YES;
                     }];
}
...
-(void)showInspireView {
    ...
    [UIView animateWithDuration:0.8
                     ...
                     animations:^{
                         [self.inspireView setFrame:frame];
                         self.inspireImageView.alpha = 1;
                     }
                     completion:^(BOOL finished){
                         CGRect toFrame = self.inspireButton.frame;
                         toFrame.origin.x = 0;
                         self.inspireButton.frame = toFrame;
                         self.inspireImageView.hidden = NO;
                     }];
}
        ...
        if(frcSection >= 0)
        {
            if (!([[self getFetchedResultsControllerForCellType:cellType] sections] == nil))
            {
                if(!(([[[self getFetchedResultsControllerForCellType:cellType] sections] count]) == 0))
                {
                    if(!(frcSection > ([[[self getFetchedResultsControllerForCellType:cellType] sections] count] - 1)))
                    {
                        _selectedResult = [[self getFetchedResultsControllerForCellType:cellType] objectAtIndexPath:[NSIndexPath indexPathForItem:indexPath.item inSection:frcSection]];
                    }
                }
            }
        }
...
- (void) actionAfterSuccessfulAnswerToRestConnection:(connectionType)connection WithResult:(NSArray *)mappingResult
{
    ...
    switch (connection)
    {
        ...
        case GET_PRODUCT:
        {
            ...
            parametersForNextVC = [NSArray arrayWithObjects: _selectedResult, selectedSpecificResult, resultContent, resultReviews, resultAvailabilities, review_array, _currentSearchQuery, nil];
