...
- (IBAction)hangerPush:(id)sender {
    if (!(thePostContent.wardrobeId == nil))
    {
        ...
        {
            [self.postDelegate openWardrobeWithId:thePostContent.wardrobeId];
        }
    }
}
...
- (void)taggableView:(GSTaggableView *)taggableView didSelectKeywords:(NSArray *)keywordArray{
    [self.postDelegate searchForKeywords:keywordArray];
}
