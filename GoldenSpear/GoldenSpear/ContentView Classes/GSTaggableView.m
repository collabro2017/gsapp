...
- (void)addTags:(NSArray*)keywords{
    ...
    for (KeywordFashionistaContent* kFC in keywords) {
        ...
        if(!groupArray){
            ...
            tagView.titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 10);
            ...
            if (kFC.keyword.userId&&![kFC.keyword.userId isEqualToString:@""]) {
                ...
                [tag setBackgroundImage:[UIImage imageNamed:@"tag-unselected.png"] forState:UIControlStateNormal];
                [tag setBackgroundImage:[UIImage imageNamed:@"tag-selected.png"] forState:UIControlStateSelected];
            }else{
            ...
            //frame.size.width += 27;
            //frame.size.width = MIN(kMaxTagSize*screenWidth, frame.size.width);
            ...
            viewFrame.origin.y = frame.origin.y + frame.size.height - 7;
            ...
            CGRect frame = tag.frame;
            //frame.size.width += 27;
            //frame.size.width = MIN(kMaxTagSize*[UIScreen mainScreen].bounds.size.width, frame.size.width);
            CGFloat screenWidth = [UIScreen mainScreen].bounds.size.width;
...
- (void)commitKeywordAction:(Keyword*)keyword{
    if (keyword.userId&&![keyword.userId isEqualToString:@""]) {
        ...
        [self.viewDelegate taggableView:self didSelectKeywords:@[[keyword toStringButtonSlideView]]];
    }
}
...
- (void)slideButtonView:(SlideButtonView *)slideButtonView btnClick:(int)buttonEntry
{
    if(!(slideButtonView.arrayButtons == nil))
    {
        if([slideButtonView.arrayButtons count] > 0)
        {
            ...
            [self.viewDelegate taggableView:self didSelectKeywords:searchTerms];
        }
    }
